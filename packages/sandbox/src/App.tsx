...
const Disclosure: FC<IDisclosure> = ({
  disabled,
  title,
  description,
}): JSX.Element => {
...
const AvatarDemo: FC<IAvatar> = ({ content }): JSX.Element => {
  return <div className="eds-avatar">{content}</div>;
};
...
const ButtonDemo: FC<IButtonDemo> = ({
  name,
  label,
  icon,
  size,
...
const Checkbox: FC<ICheckbox> = ({ label, ...extra }): JSX.Element => {
  if (label && typeof label === "string") {
    ...
    const labelClass = `eds-checkbox__label ${
      extra.disabled ? "eds-checkbox__label--disabled" : ""
    }`;
    return (
...
const Datepicker: FC<IDatepicker> = ({
  ...
  if (native) {
    return (
      <Padded>
        <input className="eds-datepicker-native" type="date" />
      </Padded>
    );
  }
  ...
  return (
    <Padded>
      <div className={root.toString()}>
        {/*TODO: refactor the disabled attribute on div */}
        {label && (
          <label className={labelCl.toString()} htmlFor={label}>
            {label}
          </label>
        )}
      ...
      <Padded>
        <div className={`${root}__container`}>
          <div className={labelRoot.toString()}>
            <label className={labelText.toString()}>{label}</label>
            {error && <div className={labelRight.toString()}>{error}</div>}
          </div>
          <div>
...
const List: FC<IList> = ({ of, children, ...extra }): JSX.Element => {
  return (
    <div className={`eds-${of}__list`} {...extra}>
      {children}
    </div>
  );
};
  ...
  return (
    <nav className={`${root}`}>
      ...
      <ul>
        ...
        {/* eslint-disable-next-line */}
        <a href="#" className={`${root}--is-active`}>
          <li>3</li>
        </a>
        {/* eslint-disable-next-line */}
        <a href="#">
        ...
        {/* eslint-disable-next-line */}
        <a href="#" className={`${root}--is-disabled`}>
          <li>5</li>
        </a>
        {/* eslint-disable-next-line */}
        <a href="#">
...
const RadioButton: FC<IRadioButton> = ({ label, ...extra }): JSX.Element => {
  if (label && typeof label === "string") {
    const id = label.replace(" ", "_");
    const labelClass = `eds-radio__label ${
      extra.disabled ? "eds-radio__label--disabled" : ""
    }`;
    return (
  ...
  return (
    <div className={root.toString()}>
      ...
      <select defaultValue="1" disabled={innerDisabled}>
        ...
        <option value={128}>
          Some really long option that will break all styles
        </option>
      </select>
    </div>
  );
    ...
    return (
      <div className={container.toString()}>
        <input
          ...
          {...extra}
        />
        <label htmlFor={id} className={labelClass.toString()}>
          {label}
        </label>
      </div>
    );
    ...
    <div>
      ...
      <section>
        ...
        <ButtonDemo
          name="Primary"
          icon="announcement"
          label="Primary Icon + Label Small"
          size="small"
        />
      ...
      <section>
        ...
        <div style={{ width: "435px" }}>
          ...
          <Padded>
            <SelectDemo
              disabledClass
              placeholder="Disabled using eds-list--disabled"
            />
          </Padded>
        </div>
      </section>
      ...
      <section>
        ...
        <h2 className="eds-typography__h2">Title h2</h2>
        <h3 className="eds-typography__h3">Title h3</h3>
        <h4 className="eds-typography__h4">Title h4</h4>
      </section>
    </div>
  );
}
